...
vector<vector<Match>> MatchCombinations::computeRank(vector<vector<Match>> matchRanks){
    vector<vector<Match>> finalVect;
    int passLevel; //declared passlevel
    int currentSetLevel; //level for each vector of matches
    for(int i = 0; i < matchRanks.size(); i++){
        passLevel = matchRanks[i].size() / 2;
        currentSetLevel = 0;
        for(int y = 0; y < matchRanks[i].size(); y++){
...
vector<Match> MatchCombinations:: computeOptimalSet(vector<vector<Match>> finalVect){
    ...
    for(int i = 0; i < finalVect.size(); i++){
        setLevel = 0;
        for(int y = 0; y < finalVect[i].size(); y++){
        ...
        if((setAvgLevel > optimalSetAvgLevel) || (setAvgLevel == optimalSetAvgLevel && finalVect[i].size >> optimalSet.size())){
            ...
            optimalSetLevel = 0;
            for(int z = 0; z < optimalSet.size(); z++){
...
vector<vector<Match>> MatchCombinations:: MakeMatchCombinations(vector<vector<string>> MatchIDcombinations, vector<vector<Match>> oneToOneMatches){
    ...
    int currentVec = 0;
    for(int i = 0; i < MatchIDcombinations.size(); i++){

        for(int y = 0; y < MatchIDcombinations[i].size(); y++){
            int id1;
            int id2;
            if(strlen(MatchIDcombinations[i][y]) % 2 == 0){
            ...
            if(oneToOneMatches[id1][id2]!=NULL){

                    //add it to the curretnVEc
